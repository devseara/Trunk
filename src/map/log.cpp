...
void log_security_audit( const map_session_data* sd, const char* action )
{
	nullpo_retv(sd);
	nullpo_retv(action);
	...
	if( ( logfp = fopen("log/security_audit.log", "a") ) == nullptr )
		return;
	time(&curtime);
	strftime(timestring, sizeof(timestring), "%Y-%m-%d %H:%M:%S", localtime(&curtime));
	fprintf(logfp, "%s\t%d\t%d\t%s\t%s\n", timestring, sd->status.account_id, sd->status.char_id, sd->status.name, action);
	fclose(logfp);
}

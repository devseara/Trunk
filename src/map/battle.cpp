...
TIMER_FUNC(battle_delay_damage_sub){
	...
	if ( dat ) {
		...
		if (sd)
			aa_pending_consume(sd, dat->target_id, dat->damage, tick);
...
int32 battle_delay_damage(t_tick tick, int32 amotion, block_list *src, block_list *target, int32 attack_type, uint16 skill_id, uint16 skill_lv, int64 damage, enum damage_lv dmg_lv, int16 div_, bool additional_effects, bool isspdamage, bool is_norm_attacked)
{
	...
	if (src->type == BL_PC) {
		...
		// après avoir déterminé la vraie échéance du timer
		t_tick due = tick + amotion;       // c’est la date à laquelle le timer tirera
		t_tick until = due + 50;           // petite marge (50 ms) pour l’ordre AA qui suit
		aa_pending_add(src, target->id, damage, until);
	}
...
static bool is_attack_critical(struct Damage* wd, block_list *src, const block_list *target, int32 skill_id, int32 skill_lv, bool first_call)
{
	...
	if( sstatus->cri )
	{
		...
		if(tsd && tsd->bonus.critical_skill_def)
			cri = cri * ( 100 - tsd->bonus.critical_skill_def ) / 100;
...
static void battle_calc_skill_base_damage(struct Damage* wd, block_list *src,block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch (skill_id) {	//Calc base damage according to skill
		...
		default:
			...
			if (tsd != nullptr && tsd->bonus.critical_skill_def != 0 && !skill_id && (bflag & BDMG_CRIT)) {
				ATK_ADDRATE(wd->damage, wd->damage2, -tsd->bonus.critical_skill_def);
			}
...
static struct Damage battle_calc_weapon_attack(block_list *src, block_list *target, uint16 skill_id, uint16 skill_lv, int32 wflag)
{
	...
	if (is_attack_critical(&wd, src, target, skill_id, skill_lv, false)) {
		...
		if (tsd && tsd->bonus.crit_def_rate != 0)
			ATK_ADDRATE(wd.damage, wd.damage2, -tsd->bonus.crit_def_rate);
		if (tsd && tsd->bonus.critical_skill_def != 0)
			ATK_ADDRATE(wd.damage, wd.damage2, -tsd->bonus.critical_skill_def);
	}
#endif
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "overkill_mode",                      &battle_config.overkill_mode,                   0,      0,      2,              },
	{ "drops_overkill",                     &battle_config.drops_overkill,                  0,      0,      INT_MAX,        },
	{ "aa_drops_penalty",                   &battle_config.aa_drops_penalty,                0,      0,      INT_MAX,        },
	{ "fake_drop_rate",                   	&battle_config.fake_drop_rate,                	0,      0,      INT_MAX,        },
	{ "sell_price_threshold",  				&battle_config.sell_price_threshold,  			499, 	0, 		INT_MAX 		},
	{ "sell_rate_below",       				&battle_config.sell_rate_below,       			60,  	0, 		100     		},
	{ "sell_rate_above",       				&battle_config.sell_rate_above,       			40,  	0, 		100     		},
...
// rAthena
	...
	{ "min_npc_warp_distance",          	&battle_config.min_npc_warp_distance,       	10,     0,      100             },
	...
	{ "feature.autoattack_hateffect",		&battle_config.feature_autoattack_hateffect,	1,		0,		1,				},
	...
	{ "feature.autoattack_pickup_delay",	&battle_config.feature_autoattack_pickup_delay,	500,   100,     INT_MAX,      	},
	...
	{ "feature.autoattack_movetype",		&battle_config.feature_autoattack_movetype,		0,		0,		2,				},
	...
	{ "feature.autoattack_drop_ratio",		&battle_config.feature_autoattack_drop_ratio,	100,	0,		INT_MAX,		},
	{ "feature.autoattack_exp_ratio",		&battle_config.feature_autoattack_exp_ratio,	100,	0,		INT_MAX,		},
	{ "feature.autoattack_allow_town",		&battle_config.feature_autoattack_allow_town,	1,		0,		1,				},
	{ "feature.autoattack_allow_pvp",		&battle_config.feature_autoattack_allow_pvp,	1,		0,		1,				},
	{ "feature.autoattack_allow_gvg",		&battle_config.feature_autoattack_allow_gvg,	1,		0,		1,				},
	{ "feature.autoattack_allow_bg",		&battle_config.feature_autoattack_allow_bg,		1,		0,		1,				},
	{ "feature.autoattack_duration_type",	&battle_config.feature_autoattack_duration_type,0,		0,		1,				},
	{ "feature.autoattack_timer",			&battle_config.feature_autoattack_timer,		250,	25,		INT_MAX,		},
	{ "feature.autoattack_iplimit",			&battle_config.feature_autoattack_iplimit,		0,		0,		INT_MAX,		},
	{ "feature.autoattack_gepardlimit",		&battle_config.feature_autoattack_gepardlimit,	0,		0,		INT_MAX,		},
	{ "feature.autoattack_mobdead_delay",	&battle_config.feature_autoattack_mobdead_delay,1000,	25,		INT_MAX,		},
	//fake players
	{ "feature.fake_enable",				&battle_config.feature_fake_enable,				1,		0,		1				},
	{ "feature.fake_max_online",			&battle_config.fake_max_online,					150,	0,		INT_MAX			},
	{ "feature.fake_allow_teleport",		&battle_config.fake_allow_teleport,				1,		0,		1				},
	{ "feature.fake_teleport_min",			&battle_config.fake_teleport_min,				30000,	300,	INT_MAX			},
	{ "feature.fake_teleport_max",			&battle_config.fake_teleport_max,				150000,	300,	INT_MAX			},
	{ "feature.fake_waypoint_repath_ms",	&battle_config.fake_waypoint_repath_ms,			800,	1,		INT_MAX			},
	{ "feature.fake_idle_sit_weight",		&battle_config.fake_idle_sit_weight,			50,		0,		1000			},
	{ "feature.fake_idle_turn_weight",		&battle_config.fake_idle_turn_weight,			30,		0,		1000			},
	{ "feature.fake_idle_move_weight",		&battle_config.fake_idle_move_weight,			20,		0,		1000			},
	{ "feature.fake_idle_step_min",			&battle_config.fake_idle_step_min,				2,		1,		20				},
	{ "feature.fake_idle_step_max",			&battle_config.fake_idle_step_max,				6,		1,		30				},
	{ "feature.fake_idle_action_min_ms",	&battle_config.fake_idle_action_min_ms,			2500,	1,		INT_MAX			},
	{ "feature.fake_idle_action_max_ms",	&battle_config.fake_idle_action_max_ms,			6000,	1,		INT_MAX			},
	{ "feature.fake_spawn_batch_min",		&battle_config.fake_spawn_batch_min,			1,		1,		1000			},
	{ "feature.fake_spawn_batch_max",		&battle_config.fake_spawn_batch_max,			2,		1,		1000			},
	{ "feature.fake_adjust_period_sec",		&battle_config.fake_adjust_period_sec,			30,		1,		3600			},
	{ "feature.fake_reject_pm",				&battle_config.fake_reject_pm,					0,		0,		1				},
	{ "feature.fake_loot_priority",			&battle_config.fake_loot_priority,				0,		0,		1				},
	//others
	...
	{ "feature.goldpc_time",                &battle_config.feature_goldpc_time,          3600,      60, INT_MAX,              },
	{ "feature.goldpc_max_points",          &battle_config.feature_goldpc_max_points,   	99999,      1, INT_MAX,              },

...
BUILDIN_FUNC(fakeIcon)
{
    ...
    clif_status_change((block_list*)sd, icon, state, time, 0, 0, 0);
...
BUILDIN_FUNC(strmobinfo)
{
	...
	case 8: script_pushstrcopy(st,mob->sprite.c_str()); break;
	default:
		script_pushint(st,0);
		break;
	}
...
// Start auto attack from rental items
...
BUILDIN_FUNC(aa_getautoattackstring)
{
	...
	case 7:
		handle_autoattack_potions(os_buf, sd);
		break;

	case 8:
		handle_autoattack_return_to_savepoint(os_buf, sd);
		break;

	case 9:
		handle_autoattack_token_of_siegfried(os_buf, sd);
		break;

	case 10:
		handle_autoattack_party_request(os_buf, sd);
		break;

	case 11:
		handle_autoattack_priorize_loot_fight(os_buf, sd);
		break;
...
BUILDIN_FUNC(aa_setautoattack)
{
	...
	switch (id) {
	case 0:
		handleAutoAttackHeal(result, sd);
		break;
	case 1:
		handleAutoAttackPotion(result, sd);
		break;
	case 2:
		handleAutoAttackSkills(result, sd);
		break;
	case 3:
		handleAutoAttackBuffSkills(result, sd);
		break;
	case 4:
		handleAutoAttackTeleportFlywing(result, sd);
		break;
	case 5:
		handleAutoAttackItems(result, sd);
		break;
	case 6:
		handleAutoAttackTeleportSkill(result, sd);
		break;
	case 7:
		handleAutoAttackMeleeAttack(result, sd);
		break;
	case 8:
		handleAutoAttackPotionState(result, sd);
		break;
	case 9:
		handleAutoAttackReturnToSavepoint(result, sd);
		break;
	case 10:
		handleAutoAttackResetConfig(sd);
		break;
	case 11:
		handleAutoAttackTokenOfSiegfried(result, sd);
		break;
	case 12:
		handleAutoAttackAcceptPartyRequest(result, sd);
		break;
	case 13:
		handleAutoAttackPriorizeLootFight(result, sd);
		break;
	case 14:
		handleAutoAttackSitRestHp(result, sd);
		break;
	case 15:
		handleAutoAttackSitRestSp(result, sd);
		break;
	case 16:
		handleAutoAttackTeleportMinHp(result, sd);
		break;
	case 17:
		handleAutoAttackTeleportNoMobMeet(result, sd);
		break;
	case 18:
		handleAutoAttackIgnoreAggressiveMonster(result, sd);
		break;
	case 19:
		handleAutoAttackMonsterSelection(result, sd);
		break;
	case 20:
		handleAutoAttackItemPickupConfiguration(result, sd);
		break;
	case 21:
		handleAutoAttackItemPickupSelection(result, sd);
		break;
	case 22:
		handleAutoAttackActionOnEnd(result, sd);
		break;
	case 23:
		handleAutoAttackMonsterSurround(result, sd);
		break;
	}
...
BUILDIN_FUNC(mesf)
{
    ...
    const char* fmt = script_getstr(st, 2);

    // Prepare a va_list manually (max 16 args)
    // We'll store both strings and ints
    intptr_t args[16]; // intptr_t so it can hold either pointer or int
    int argCount = 0;

    for (int i = 3; i <= script_lastdata(st) && argCount < 16; i++) {
        struct script_data* data = script_getdata(st, i);
        if (data_isstring(data)) {
            args[argCount++] = (intptr_t)script_getstr(st, i);
        } else {
            args[argCount++] = (intptr_t)script_getnum(st, i);
        }
    }

    // This part is tricky: we can't pass variable number of args directly,
    // so we'll just handle up to 16 manually
    switch (argCount) {
        ...
        case 4: snprintf(buf, sizeof(buf), fmt, args[0], args[1], args[2], args[3]); break;
        // ... repeat for up to 16 args
        default: snprintf(buf, sizeof(buf), "%s", fmt); break;
    }

    clif_scriptmes(*sd, st->oid, buf);
    return SCRIPT_CMD_SUCCESS;
}

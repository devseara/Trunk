...
static int32 clif_send_sub(block_list *bl, va_list ap)
{
	...
	if (RBUFW(buf, 0) == 0x8d && sd->fp.is_fake_player) {
		// --- FakeChat hook: intercept ZC_NOTIFY_CHAT (global/local) vers destinataires FAKE ---
		do {
			// 1) Header
			if (len < 2) break;
			uint16 pkt = WBUFW((uint8*)buf, 0);
			// match 0x8D et 0x8C (selon branche)
			if (!(pkt == HEADER_ZC_NOTIFY_CHAT || pkt == 0x008C)) break;

			// 2) Type de diffusion : on cible bien l’aire publique
			if (type != AREA_WOC && type != AREA_CHAT_WOC && type != AREA_WOS && type != AREA_WOSC) {
				//ShowError("[FakeChat][HookSend] skip type=%d pkt=0x%04x", type, pkt);
				break;
			}

			// 3) Émetteur = joueur ? (sinon ignore)
			map_session_data* from_sd = nullptr;
			if (src_bl && src_bl->type == BL_PC)
				from_sd = (map_session_data*)src_bl;
			if (!from_sd) break;

			// 4) Extraire message (après le pseudo et " : ")
			size_t off = offsetof(PACKET_ZC_NOTIFY_CHAT, Message);
			if ((int32)off >= len) break;
			const char* raw = (const char*)((const uint8*)buf + off);
			if (!raw || !*raw) break;

			// Certains builds n’ont pas forcément le \0 fiable -> borne par la longueur totale
			size_t avail = len - off;
			std::string msg_full(raw, strnlen(raw, avail)); // "Name : message" ou parfois juste "message"

			// Séparer "Name : " si présent
			const char* sep = strstr(msg_full.c_str(), " : ");
			const char* usermsg = sep ? sep + 3 : msg_full.c_str();

			// Trim
			while (*usermsg == ' ' || *usermsg == '\t' || *usermsg == '\n' || *usermsg == '\r') ++usermsg;
			if (!*usermsg) break;

			// 5) Appel pipeline
			std::string reply, intent; int delay_ms = 0;
			bool handled = fakechat_try_handle_message_step2(
				/*bot=*/sd, /*from=*/from_sd, /*msg=*/std::string(usermsg),
				/*is_private=*/false,
				/*out*/reply, delay_ms, intent,
				/*channel_hint=*/FCChannel::GLOBAL /* ou FCChannel::GLOBAL si tu veux forcer */,
				/*target_id_hint=*/0
			);
		} while (0);
	}
...
int32 clif_spawn( const block_list* bl, bool walking ){
	...
	if( bl->type == BL_PC )
		clif_extended_name(bl, nullptr, AREA);
...
void clif_selllist( const map_session_data& sd){
	...
	for( int32 i = 0, c = 0; i < MAX_INVENTORY; i++ ){
		...
		/* [DevSeara]
            Sell Modify Rate.
        */
        int32 final_price = pc_modifysellvalue(&sd, price);

        if (sd.inventory_data[i]->type == IT_WEAPON || sd.inventory_data[i]->type == IT_ARMOR) {
            final_price = 20;
        } else {
            if (price <= battle_config.sell_price_threshold) {
                final_price -= (price * battle_config.sell_rate_below) / 100;
            } else {
                final_price -= (price * battle_config.sell_rate_above) / 100;
            }
        }
        ...
        packet->items[c].price = final_price;       
        packet->items[c].overcharge = final_price;
...
void clif_name( const block_list* src, const block_list* bl, send_target target ){
	...
	clif_extended_name(bl, src, target);
}

static const block_list* clif_extended_name_anchor(const block_list* bl, const block_list* receiver, send_target target) {
	if (target == SELF && receiver != nullptr)
		return receiver;
	return bl;
}

static void clif_extended_name_unit_title(PACKET_ZC_EXT_NAME& packet, const block_list* bl) {
	const unit_data* ud = unit_bl2ud(bl);
	if (ud == nullptr || ud->title[0] == '\0')
		return;

	safestrncpy(packet.title_name, ud->title, DISPLAY_NAME_LENGTH);
	safestrncpy(packet.title_name_short, ud->title, NAME_LENGTH);
}

static void clif_extended_name_pc(const block_list* bl, const block_list* receiver, send_target target) {
	const map_session_data* sd = static_cast<const map_session_data*>(bl);
	if (sd->fakename[0] != '\0')
		return;

	PACKET_ZC_EXT_NAME packet{};
	packet.packetType = HEADER_ZC_EXT_NAME;
	packet.packetLength = sizeof(packet);
	packet.magic = 0x4558544E; // EXTN
	packet.gid = bl->id;
	packet.type = 0;
	safestrncpy(packet.shortName, sd->status.name, NAME_LENGTH);
	if (sd->status.display_name[0] != '\0')
		safestrncpy(packet.name, sd->status.display_name, DISPLAY_NAME_LENGTH);

	party_data* party = sd->status.party_id != 0 ? party_search(sd->status.party_id) : nullptr;
	if (party != nullptr && (sd->guild != nullptr || battle_config.display_party_name)) {
		safestrncpy(packet.party_name, party->party.name, DISPLAY_NAME_LENGTH);
		safestrncpy(packet.party_name_short, party->party.name, NAME_LENGTH);
	}

	if (sd->guild != nullptr) {
		const int32 position = guild_getposition(*sd);
		safestrncpy(packet.guild_name, sd->guild->guild.name, DISPLAY_NAME_LENGTH);
		safestrncpy(packet.guild_name_short, sd->guild->guild.name, NAME_LENGTH);
		safestrncpy(packet.position_name, sd->guild->guild.position[position].name, DISPLAY_NAME_LENGTH);
		safestrncpy(packet.position_name_short, sd->guild->guild.position[position].name, NAME_LENGTH);
	} else if (sd->clan != nullptr) {
		safestrncpy(packet.position_name, sd->clan->name, DISPLAY_NAME_LENGTH);
		safestrncpy(packet.position_name_short, sd->clan->name, NAME_LENGTH);
	}

	if (pc_get_group_id(sd) > 0 && sd->group != nullptr) {
		safestrncpy(packet.title_name, sd->group->name.c_str(), DISPLAY_NAME_LENGTH);
		safestrncpy(packet.title_name_short, sd->group->name.c_str(), NAME_LENGTH);
	}

	if (packet.name[0] != '\0' || packet.party_name[0] != '\0' || packet.guild_name[0] != '\0' ||
		packet.position_name[0] != '\0' || packet.title_name[0] != '\0')
		clif_send(&packet, sizeof(packet), clif_extended_name_anchor(bl, receiver, target), target);
}

static void clif_extended_name_mob(const block_list* bl, const block_list* receiver, send_target target) {
	const mob_data* md = static_cast<const mob_data*>(bl);
	PACKET_ZC_EXT_NAME packet{};
	packet.packetType = HEADER_ZC_EXT_NAME;
	packet.packetLength = sizeof(packet);
	...
	packet.gid = bl->id;
	packet.type = 1;
	safestrncpy(packet.shortName, md->name, NAME_LENGTH);
	if (md->db != nullptr && !md->db->display_name.empty())
		safestrncpy(packet.name, md->db->display_name.c_str(), DISPLAY_NAME_LENGTH);
	clif_extended_name_unit_title(packet, bl);

	if (packet.name[0] != '\0' || packet.title_name[0] != '\0')
		clif_send(&packet, sizeof(packet), clif_extended_name_anchor(bl, receiver, target), target);
}

static void clif_extended_name_npc(const block_list* bl, const block_list* receiver, send_target target) {
	const npc_data* nd = static_cast<const npc_data*>(bl);
	PACKET_ZC_EXT_NAME packet{};
	packet.packetType = HEADER_ZC_EXT_NAME;
	packet.packetLength = sizeof(packet);
	packet.magic = 0x4558544E; // EXTN
	packet.gid = bl->id;
	packet.type = nd->subtype == NPCTYPE_WARP ? 3 : 2;

	const size_t stockLength = strcspn(nd->name, "#");
	safesnprintf(packet.shortName, sizeof(packet.shortName), "%.*s",
		static_cast<int32>(std::min(stockLength, static_cast<size_t>(NAME_LENGTH - 1))), nd->name);
	if (nd->display_name[0] != '\0') {
		safestrncpy(packet.name, nd->display_name, DISPLAY_NAME_LENGTH);
	} else if (stockLength >= NAME_LENGTH) {
		safesnprintf(packet.name, sizeof(packet.name), "%.*s", static_cast<int32>(stockLength), nd->name);
	}
	clif_extended_name_unit_title(packet, bl);

	// AlwaysShowNpcName uses type-2 EXTN packets as its NPC discovery stream,
	// including NPCs whose stock 24-byte name needs no extended replacement.
	// Portal NPCs remain type 3 and are intentionally excluded by the client.
	if (packet.type == 2 || packet.name[0] != '\0' || packet.title_name[0] != '\0')
		clif_send(&packet, sizeof(packet), clif_extended_name_anchor(bl, receiver, target), target);
}

void clif_extended_name(const block_list* bl, const block_list* receiver, send_target target) {
	nullpo_retv(bl);
	if (!battle_config.feature_custom_extended_name_packet)
		return;

	switch (bl->type) {
		case BL_PC:
			clif_extended_name_pc(bl, receiver, target);
			break;
		case BL_MOB:
			clif_extended_name_mob(bl, receiver, target);
			break;
		case BL_NPC:
			clif_extended_name_npc(bl, receiver, target);
			break;
		default:
			break;
	}
...
static bool clif_process_message(map_session_data* sd, bool whisperFormat, char* out_name, char* out_message, char* out_full_message ){
	...
	if (strchr(out_full_message, '\n') != NULL ||
	    strchr(out_full_message, '\r') != NULL ||
	    strstr(out_full_message, "             ") != NULL) {
		return false;
	}
...
void clif_parse_WisMessage(int32 fd, map_session_data* sd)
{
	...
	if (dstsd->state.autotrade == 1 && !dstsd->fp.is_fake_player){
	...
	if (dstsd->fp.is_fake_player) {
		std::string_view sv{ message };
		constexpr char ws[] = " \t\n\r\f\v";

		// Trim in-place via string_view
		std::string_view so; // trimmed
		if (const auto first = sv.find_first_not_of(ws); first != std::string_view::npos) {
			const auto last = sv.find_last_not_of(ws);
			so = sv.substr(first, last - first + 1);
		}

		if (!so.empty()) {
			// Pour chaque faux joueur candidat "bot_sd" (à toi de boucler selon ta liste / map / range)
			std::string reply, intent;
			int delay_ms = 0;

			if (fakechat_try_handle_message_step2(dstsd, sd, std::string(so), true,
				reply, delay_ms, intent,
				FCChannel::PM /*hint*/, sd->id /*target*/))
			{
				ShowInfo("[FakeChat] from='%s' -> bot='%s' intent=%s delay=%dms reply=\"%s\" \n",
					sd->status.name, dstsd->status.name, intent.c_str(), delay_ms, reply.c_str());
				// Étape 1 : on ne l’envoie pas encore, on vérifie juste les logs.
			}
		}
	}
...
static TIMER_FUNC(clif_shop_showscript_refresh){
	map_session_data* sd = map_id2sd( id );
	if( !sd || sd->npc_shopid <= 0 ){
		if( sd ){
			sd->shop_showscript_tid = INVALID_TIMER;
		}
		delete_timer( tid, clif_shop_showscript_refresh );
		return 0;
	}
	npc_data* nd = map_id2nd( sd->npc_shopid );
	if( !nd || ( nd->subtype != NPCTYPE_POINTSHOP && nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_ITEMSHOP ) ){
		sd->shop_showscript_tid = INVALID_TIMER;
		delete_timer( tid, clif_shop_showscript_refresh );
		return 0;
	}
	...
	char output[CHAT_SIZE_MAX];
	bool send_script = false;
	switch( nd->subtype ) {
		case NPCTYPE_POINTSHOP: {
			const char* currency_name;
			if( strstr( nd->exname, "GOLDPCCAFE" ) != nullptr && strcmp( nd->u.shop.pointshop_str, GOLDPC_POINT_VAR ) == 0 )
				currency_name = "Gold PC Points";
			else
				currency_name = nd->u.shop.pointshop_str;
			safesnprintf( output, sizeof(output), msg_txt( sd, 10350 ), currency_name, sd->status.name, currency_name, rathena::util::insert_comma( cost[0] ).c_str() );
			send_script = true;
			break;
		}
		case NPCTYPE_CASHSHOP:
			safesnprintf( output, sizeof(output), msg_txt( sd, 10351 ), rathena::util::insert_comma( cost[0] ).c_str(), rathena::util::insert_comma( cost[1] ).c_str() );
			send_script = true;
			break;
		case NPCTYPE_ITEMSHOP: {
			std::shared_ptr<item_data> item = item_db.find( nd->u.shop.itemshop_nameid );
			if( item ) {
				safesnprintf( output, sizeof(output), msg_txt( sd, 10352 ), item->ename.c_str(), item->ename.c_str(), rathena::util::insert_comma( cost[0] ).c_str() );
				send_script = true;
			}
			break;
		}
	...
	if( send_script ){
		clif_showscript_to_player( sd, nd->id, output );
	}else{
		sd->shop_showscript_tid = INVALID_TIMER;
		delete_timer( tid, clif_shop_showscript_refresh );
	}
...
void clif_shop_showscript_clear( map_session_data* sd ){
	nullpo_retv( sd );
	if( sd->shop_showscript_tid != INVALID_TIMER ){
		delete_timer( sd->shop_showscript_tid, clif_shop_showscript_refresh );
		sd->shop_showscript_tid = INVALID_TIMER;
	}
	if( sd->npc_shopid > 0 ){
		clif_showscript_to_player( sd, sd->npc_shopid, "" );
	}
}
...
void clif_cashshop_show( map_session_data& sd, const npc_data& nd ){
	...
	// Showscript: display currency type and balance above NPC (removed when shop closes)
	{
		char output[CHAT_SIZE_MAX];
		bool send_script = false;
		...
		if( send_script ){
			clif_showscript_to_player( &sd, nd.id, output );
			// Refresh every 2s to keep visible (client may hide after a few seconds)
			if( sd.shop_showscript_tid != INVALID_TIMER ){
				delete_timer( sd.shop_showscript_tid, clif_shop_showscript_refresh );
			}
			sd.shop_showscript_tid = add_timer_interval( gettick() + 2000, clif_shop_showscript_refresh, sd.id, 0, 2000 );
			if( sd.shop_showscript_tid == INVALID_TIMER ){
				ShowWarning( "clif_cashshop_show: Failed to create shop showscript refresh timer for %s (%d).\n", sd.status.name, sd.id );
			}
		}
	}
}
...
/**
 * Send showscript to a specific player with a specific block id (for NPC display).
 * Use when the text should appear above an NPC but only the player should see it.
 */
void clif_showscript_to_player( map_session_data* sd, int32 block_id, const char* message ){
	char buf[256];
	size_t len;
	nullpo_retv( sd );

	len = message ? strlen(message)+1 : 1;
	if( len > sizeof(buf)-8 ) {
		ShowWarning("clif_showscript_to_player: Truncating too long message (len=%" PRIuPTR ").\n", len);
		len = sizeof(buf)-8;
	}

	WBUFW(buf,0) = 0x8b3;
	WBUFW(buf,2) = (uint16)(len+8);
	WBUFL(buf,4) = block_id;
	char* msg_dst = WBUFCP(buf,8);
	safestrncpy(msg_dst, message ? message : "", len);
	// Replace literal \n (from msg_conf) with actual newline for client display
	for( char* p = msg_dst; ( p = strstr( p, "\\n" ) ) != nullptr; p++ ){
		*p = '\n';
		memmove( p + 1, p + 2, strlen( p + 2 ) + 1 );
	}
	WBUFW(buf,2) = (uint16)(strlen(msg_dst) + 1 + 8);
	clif_send( (unsigned char*)buf, WBUFW(buf,2), sd, SELF );
}
...
void clif_roulette_open( map_session_data* sd ){
    ...
    npc_event_do_id("Autoattack::OnStart", sd->status.account_id);
}
...
void clif_goldpc_info( map_session_data& sd ){
	...
	// Client typically expects 3600s (1h) scale for progress bar
	...
	if( battle_config.feature_goldpc_active ){
		...
		{
			int64 pts = pc_readparam( &sd, SP_GOLDPC_POINTS );
			p.point = (int32)cap_value( pts, 0, battle_config.feature_goldpc_max_points );
		}
		if( sd.goldpc_tid != INVALID_TIMER ){
			...
			if( td != nullptr ){
				...
				// Round up to full second so client doesn't show 3599 then jump
				if( remaining > 0 )
					remaining += ( remaining % 1000 );
				else
					remaining = 0;
				int32 secs_left = (int32)( remaining / 1000 );
				p.playedTime = (int32)( client_max_seconds - secs_left );
				p.playedTime = cap_value( p.playedTime, 0, client_max_seconds );
			}else{
				p.playedTime = 0;
			}
		}else{
...
// (^~_~^) Gepard Shield End
	...
	if ((int32)RFIFOREST(fd) < packet_len){
		if (sd == NULL) {
			int32 cmd3 = RFIFOW(fd, 0);
			if (packet_db[cmd3].func != clif_parse_WantToConnection) {
				ShowWarning("clif_parse: Received first unsupported packet (packet 0x%04x (0x%04x), %d bytes received), disconnecting session #%d.\n",
			            (unsigned int)cmd, cmd3, RFIFOREST(fd), fd);
				set_eof( fd );
			}
		}
...
void do_init_clif(void) {
	...
	add_timer_func_list(clif_shop_showscript_refresh, "clif_shop_showscript_refresh");

...
static int32 atcommand_reloaditemtransparencydb(const int32 fd, map_session_data* sd, const char* command, const char* message);
...
ACMD_FUNC( reload ){
	static const struct{
		...
		{ "itemtransparencydb", atcommand_reloaditemtransparencydb },
...
ACMD_FUNC(reloaditemtransparencydb){
	nullpo_retr(-1, sd);

	do_reload_item_transparency();
	clif_displaymessage(fd, "Item Transparency database has been reloaded.");
...
{
	int n = 5000; // défaut
	if (message && *message) n = atoi(message);
#ifdef AA_WALK_DEBUG
	aa_benchmark_paths(sd, n, 20 /*min manhattan*/, 1337 /*seed*/);
#endif
	return true;
}
...
ACMD_FUNC(addfake) {
	if (!message || !*message) {
		clif_displaymessage(sd->fd, "Usage: @addfake <count> <profile>");
		return -1;
	}
	char cstr[16] = { 0 }, pstr[64] = { 0 };
	if (sscanf(message, "%15s %63[^\r\n]", cstr, pstr) < 2) {
		clif_displaymessage(sd->fd, "Usage: @addfake <count> <profile>");
		return -1;
	}
	long cnt = strtol(cstr, nullptr, 10);
	if (cnt <= 0 || cnt > 10000) {
		clif_displaymessage(sd->fd, "Count must be in 1..10000");
		return -1;
	}
	if (!battle_config.feature_fake_enable) {
		clif_displaymessage(sd->fd, "feature.fake_enable is OFF.");
		return -1;
	}
	int spawned = fake_add_from_profile_name(std::string(pstr), (uint16)cnt);
	char buf[128];
	if (spawned > 0) snprintf(buf, sizeof(buf), "Spawned %d fake(s) on profile '%s'.", spawned, pstr);
	else snprintf(buf, sizeof(buf), "No fake spawned (unknown profile / capacity / error).");
	clif_displaymessage(sd->fd, buf);
	return 0;
}
...
ACMD_FUNC(delfake) {
	if (!message || !*message) {
		clif_displaymessage(sd->fd, "Usage: @delfake <count> <profile>");
		return -1;
	}
	char cstr[16] = { 0 }, pstr[64] = { 0 };
	if (sscanf(message, "%15s %63[^\r\n]", cstr, pstr) < 2) {
	...
	int removed = fake_cancel_pending_from_profile_name(std::string(pstr), (uint16)cnt);

	char buf[128];
	if (removed > 0)
		snprintf(buf, sizeof(buf), "Cancelled %d pending fake(s) on profile '%s'.", removed, pstr);
	else
		snprintf(buf, sizeof(buf), "No pending fake cancelled (unknown profile / none pending).");
	clif_displaymessage(sd->fd, buf);
	return 0;
}
...
ACMD_FUNC(fakecurve)
{
	if (!message || !*message) {
		clif_displaymessage(sd->fd,
			"Usage: @fakecurve <profile> [status|tick] | @fakecurve enable <profile> | @fakecurve disable <profile> | @fakecurve set <profile> <percLow> <percHigh> <smoothMin> <weekendBoost>");
		return -1;
	}

	char a[16] = { 0 }, b[64] = { 0 }, c[16] = { 0 }, d[16] = { 0 }, e[16] = { 0 }, f[32] = { 0 };
	int n = sscanf(message, "%15s %63s %15s %15s %15s %31s", a, b, c, d, e, f);
	if (n <= 0) {
		clif_displaymessage(sd->fd, "Bad params.");
		return -1;
	}

	// Sous-commandes modernes: enable/disable/set ...
	if (!strcasecmp(a, "enable")) {
		if (n < 2) { clif_displaymessage(sd->fd, "Usage: @fakecurve enable <profile>"); return -1; }
		bool ok = fakecurve_enable_profile(std::string(b), true);
		clif_displaymessage(sd->fd, ok ? "Curve enabled." : "Enable failed.");
		if (ok) fakecurve_tick_once(); // applique un 1er ajustement
		return 0;
	}
	if (!strcasecmp(a, "disable")) {
		if (n < 2) { clif_displaymessage(sd->fd, "Usage: @fakecurve disable <profile>"); return -1; }
		bool ok = fakecurve_enable_profile(std::string(b), false);
		clif_displaymessage(sd->fd, ok ? "Curve disabled." : "Disable failed.");
		return 0;
	}
	if (!strcasecmp(a, "set")) {
		if (n < 6) {
			clif_displaymessage(sd->fd, "Usage: @fakecurve set <profile> <percLow> <percHigh> <smoothMin> <weekendBoost>");
			return -1;
		}
		char* end = nullptr;
		unsigned long pl = strtoul(c, &end, 10);
		unsigned long ph = strtoul(d, &end, 10);
		unsigned long sm = strtoul(e, &end, 10);
		double wkb = strtod(f, &end);

		bool ok = fakecurve_set_profile(std::string(b), (uint16)pl, (uint16)ph, (uint16)sm, wkb);
		clif_displaymessage(sd->fd, ok ? "Curve params updated." : "Set failed.");
		if (ok) fakecurve_tick_once(); // ajuste immédiatement (sans brutalité, via smoothing)
		return 0;
	}

	// Compat historique: "<profile> status|tick"
	// a = profile, b = [status|tick] (optionnel)
	auto prof = fake_players_db.find(std::string(a));
	if (!prof) {
		clif_displaymessage(sd->fd, "Unknown profile.");
		return -1;
	}

	const int cap = fakecurve_capacity(prof.get());
	const int on = fakecurve_online(prof.get());
	const int tgt = fakecurve_target_now(prof.get());
	const int need_add = std::max(0, tgt - on);

	if (n == 1 || !strcasecmp(b, "status")) {
		char buf[256];
		snprintf(buf, sizeof(buf),
			"[Curve] prof='%s' enabled=%d cap=%d online=%d target=%d need_add=%d",
			a, (int)prof->curve.enabled, cap, on, tgt, need_add);
		clif_displaymessage(sd->fd, buf);
		return 0;
	}
	if (!strcasecmp(b, "tick")) {
		if (!prof->curve.enabled) {
			clif_displaymessage(sd->fd, "Curve disabled for this profile.");
			return 0;
		}
		// Un tick complet (ajouts et retraits gérés dans fakecurve_tick_once)
		fakecurve_tick_once();
		char buf[256];
		snprintf(buf, sizeof(buf),
			"[Curve] tick requested for '%s' (cap=%d).", a, cap);
		clif_displaymessage(sd->fd, buf);
		return 0;
	}
...
ACMD_FUNC(fakecurveproj) {
	if (!message || !*message) {
		clif_displaymessage(sd->fd, "Usage: @fakecurveproj <profile>");
		return -1;
	}
	auto prof = fake_players_db.find(std::string(message));
	if (!prof) {
		clif_displaymessage(sd->fd, "Unknown profile.");
		return -1;
	}
	fakecurve_print_24h(prof.get());
	clif_displaymessage(sd->fd, "OK: see console for 24h projection.");
	return 0;
}
...
{
	if (!message || !*message) {
		clif_displaymessage(sd->fd, "Usage: @fakeplayer <on|off>");
		return -1;
	}
	char opt[8] = { 0 };
	if (sscanf(message, "%7s", opt) < 1) {
		clif_displaymessage(sd->fd, "Usage: @fakeplayer <on|off>");
		return -1;
	}

	if (!strcasecmp(opt, "on")) {
		bool ok = fakeplayer_set_enabled(true);
		clif_displaymessage(sd->fd, ok ? "Fake players ENABLED. Smooth ramp to target." : "Failed to enable.");
		return 0;
	}
	if (!strcasecmp(opt, "off")) {
		bool ok = fakeplayer_set_enabled(false);
		clif_displaymessage(sd->fd, ok ? "Fake players DISABLED. All kicked; no reconnect." : "Failed to disable.");
		return 0;
	}
...
ACMD_FUNC(faketraining)
{
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter))
	{
		if (pl_sd->fp.tg.active && pl_sd->fp.tg.step == TGStep::WAIT)
		{
			pl_sd->fp.tg.step = TGStep::GO_WARP;
			pl_sd->fp.tg.nextActionTick = gettick() + irand(TG_DELAY[(int)TGStep::GO_WARP].minMs, TG_DELAY[(int)TGStep::GO_WARP].maxMs);
			count++;
		}
	}
	mapit_free(iter);

	sprintf(atcmd_output, "Started %d fake players in training ground moving", count);
	clif_displaymessage(fd, atcmd_output);
	return 0;
}
...
#include <custom/atcommand_def.inc>
		...
		ACMD_DEF(reloaditemtransparencydb),
		...
		ACMD_DEF(delfake),
		ACMD_DEF(fakecurve),
		ACMD_DEF(fakecurveproj),
		ACMD_DEF(fakeplayer),
		ACMD_DEF(faketraining),
	};

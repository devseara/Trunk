...
#include "fake_players.hpp" //BotChatMemory 
...
#define ACHIEVEMENTLEVEL "AchievementLevel"
	...
	#define GOLDPC_POINT_VAR "#ComodoPlaytime"
#endif
#ifndef GOLDPC_SECONDS_VAR
	#define GOLDPC_SECONDS_VAR "#HourlySeconds"
#endif
...
struct s_fake_player {
	bool is_fake_player = false;
	int32 fake_player_timer = INVALID_TIMER;
	int32 fake_player_chat = INVALID_TIMER;
	int32 fake_player_tg_quit = INVALID_TIMER;
	time_t last_tp;
	struct s_fake_profile* prof = nullptr;
	struct s_fake_jobprofil* jobprofile = nullptr;

	t_tick death_window_start = 0;
	uint32 death_count = 0;
	t_tick next_idle_action_tick = 0;

	struct BotChatMemory mem;
	enum e_fake_behavior behavior;
...
public:
	...
	struct s_state {
		...
		uint32 dead_sit : 2;
		uint32 security : 1; // @security [Cydh]
		e_lr_flag lr_flag;
...
int32 pc_calc_sell_price( const map_session_data*, const struct item_data*, int32 base_price );

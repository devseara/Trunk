...
#define AA_PENDING_SLOTS 8
#define AA_PEND_TTL_MS       3000             // TTL maximum (3s)
#define AA_PEND_CAP          INT64_C(900000000000000000)
...
// ---------- Types / signatures ----------
typedef bool (*AAValidateBlFn)(map_session_data* sd, struct block_list* bl);

enum AABucketKind {
	AA_BUCKET_GENERIC, // md->block (ITEM & co)
	AA_BUCKET_MOB      // md->block_mob
};
...
struct aa_pending_entry {
	unsigned int target_id;   // 0 = slot vide
	int64 damage;             // somme des dégâts retardés restants vers cette cible
	t_tick until;             // tick d’expiration (timer prévu + petite marge)
};

// Structure contenant les données d'entrée et de sortie pour un calcul
struct AsyncPathJob {
	// Identifiants pour retrouver le joueur plus tard
	int account_id;
	int char_id;

	// Données nécessaires au calcul (copies strictes)
	int16 m;
	int start_x, start_y;
	int target_x, target_y;

	// Résultat
	std::atomic<bool> is_done{ false }; // <- atomic
	bool is_fake = false;
	bool success = false;      // A-t-on trouvé un chemin ?
	std::vector<std::tuple<int, int>> path; // Le chemin trouvé
};
...
struct s_autoattack {
	...
	int skill_range;
	t_tick pickup_lock_until;
	t_tick attack_lock_until;
	t_tick easy_fail_until;
	uint8 defer_on_end;
	struct aa_pending_entry pending[AA_PENDING_SLOTS];
	t_tick mob_dead_delay;
	std::shared_ptr<AsyncPathJob> current_job;
};

static unsigned int aa_find_first_bl_centripetal(map_session_data* sd, int max_radius, bool wall_check, int32 required_type, AABucketKind bucket_kind, AAValidateBlFn validate_bl);
...
#ifdef AA_WALK_DEBUG
void aa_benchmark_paths(map_session_data* sd, int num_calls,
	int min_dist = 20, uint32_t seed = 12345);
#endif

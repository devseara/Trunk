...
void pc_delinvincibletimer(map_session_data* sd)
{
	...
	// Avoid mob teleport when hit after a teleport from autoattack
...
void pc_set_display_name(map_session_data* sd, const char* display_name, bool save) {
	nullpo_retv(sd);

	safestrncpy(sd->status.display_name, display_name != nullptr ? display_name : "", DISPLAY_NAME_LENGTH);
	clif_name_area(sd);
...
uint8 pc_isequip( const map_session_data* sd, int32 n )
{
	...
	/***********************************/
	struct map_data *mapdata = map_getmapdata(sd->m);
	//ShowError("a - mapdata->noitemlist.size %d \n",mapdata->noitemlist.size());
	if(mapdata->noitemlist.size() > 0){
		...
		if (itNoitemlist != mapdata->noitemlist.end()){
			...
			if (itNoitemlist != mapdata->noitemlist.end()){
				//ShowError("c - found type %d \n", it);
				return ITEM_EQUIP_ACK_FAIL;
			}
			
			if (!itemdb_isspecial(sd->inventory.u.items_inventory[n].card[0])){
				for (slot = 0; slot < MAX_SLOTS; ++slot) {
					int cardid = sd->inventory.u.items_inventory[n].card[slot];
					//ShowError("checking slot %d card %d \n", slot,cardid);
					if(cardid){
						itNoitemlist = std::find_if(mapdata->noitemlist.begin(), mapdata->noitemlist.end(), [cardid] ( int const &v) {return v == cardid;});
						if (itNoitemlist != mapdata->noitemlist.end())
							return ITEM_EQUIP_ACK_FAIL;
					}
				}
...
TIMER_FUNC(pc_goldpc_update){
	...
	if( points < 0 ){
		points = 0; // Guard against read error or corrupted reg
	}
	...
	// Cap to max (pc_setparam will also cap, but avoid overflow)
	if( points > battle_config.feature_goldpc_max_points ){
		points = battle_config.feature_goldpc_max_points;
	}

	// Reset the seconds for this interval
...
// (^~_~^) Color Nicks End
	...
	if( battle_config.feature_goldpc_active && pc_readreg2( sd, GOLDPC_POINT_VAR ) < battle_config.feature_goldpc_max_points && !sd->state.autotrade ){
		int32 elapsed = (int32)cap_value( pc_readreg2( sd, GOLDPC_SECONDS_VAR ), 0, battle_config.feature_goldpc_time );
		int32 delay_sec = battle_config.feature_goldpc_time - elapsed;
		if( delay_sec < 0 )
			delay_sec = 0;
		t_tick delay_ms = (t_tick)delay_sec * 1000;
		sd->goldpc_tid = add_timer( gettick() + delay_ms, pc_goldpc_update, sd->id, (intptr_t)nullptr );
#ifndef VIP_ENABLE
		clif_goldpc_info( *sd );
#endif
	}else{
...
void pc_bonus(map_session_data *sd,int32 type,int32 val)
{
	...
	switch(type){
		...
		case SP_ADD_ITEM_SPHEAL_RATE:
			...
			break;
		case SP_CRITICAL_SKILL_DEF:
			if(sd->state.lr_flag != 2)
				sd->bonus.critical_skill_def += val;
			break;
		default:
...
/*==========================================
 * Calculate final sell price with equipment cap and tax
 * Applies sell_equip_max cap for equipment, sell_tax for other items
 *------------------------------------------*/
int32 pc_calc_sell_price(const map_session_data* sd, const struct item_data* id, int32 base_price)
{
	int32 val = pc_modifysellvalue(sd, base_price);

	if (id == nullptr)
		return val;

	// Equipment cap: weapons/armor/shadow gear limited to sell_equip_max
	if (battle_config.sell_equip_max > 0 &&
	    (id->type == IT_WEAPON || id->type == IT_ARMOR || id->type == IT_SHADOWGEAR) &&
	    val > battle_config.sell_equip_max) {
		val = battle_config.sell_equip_max;
	}
	// Tax on non-equipment items
	else if (battle_config.sell_tax_enable &&
	         id->type != IT_WEAPON && id->type != IT_ARMOR && id->type != IT_SHADOWGEAR) {
		int32 keep_rate = (base_price <= battle_config.sell_tax_threshold)
		                 ? battle_config.sell_tax_keep_rate_low
		                 : battle_config.sell_tax_keep_rate_high;
		val = (int32)((double)val * (double)keep_rate / 100.);
		if (val < battle_config.min_shop_sell)
			val = battle_config.min_shop_sell;
	}
...
int64 pc_readparam( const map_session_data* sd, int64 type )
{
	...
	switch(type) {
		...
		case SP_CRITICAL_SKILL_DEF: val = sd->bonus.critical_skill_def; break;
		...
		for( i = 0; i < MAX_INVENTORY; i++ ) {
			...
			if (itNoitemlist != mapdata->noitemlist.end()){
				...
				if (!itemdb_isspecial(sd->inventory.u.items_inventory[i].card[0])){
					for (slot = 0; slot < MAX_SLOTS; ++slot) {
						cardid = sd->inventory.u.items_inventory[i].card[slot];
						if(cardid){
							itNoitemlist = std::find_if(mapdata->noitemlist.begin(), mapdata->noitemlist.end(), [cardid] ( int const &v) {return v == cardid;});
							if (itNoitemlist != mapdata->noitemlist.end()){
								pc_unequipitem(sd, i, 2);
...
/** [Cydh] @security - delegates to centralized security module */
bool pc_check_security_(const class map_session_data* sd, enum e_security_check flag, bool notice) {
	return security_check(sd, flag, notice);
}
...
TIMER_FUNC(pc_autotrade_timer){
	...
	if (!sd->vender_id && !sd->buyer_id && !sd->fp.is_fake_player) {

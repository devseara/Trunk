	...
	for( i = 0; i < n; ++i ){
		...
		for (uint32 j = 0; j < quantity; j += get_amt) {
			...
			if (id) {
				...
				const char* itemName = id->ename.c_str();
				if (!itemName || itemName[0] == '\0')
					itemName = "Unknown Item";

				if (get_amt > 1) {
					snprintf(msg_a, sizeof(msg_a),
						"%s has purchased x%d %s from the Cash Shop!",
						sd->status.name, get_amt, itemName);
				} else {
					snprintf(msg_a, sizeof(msg_a),
						"%s has purchased %s from the Cash Shop!",
						sd->status.name, itemName);
				}

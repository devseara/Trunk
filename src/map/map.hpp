...
#define MAX_FP_ON_TARGET 1
struct FCClaimSlot {
	uint32 bot_aid = 0;      // account_id du fake player (ou 0 = libre)
	uint64 until = 0;     // tick d’expiration (gettick())
};
...
struct flooritem_data : public block_list {
	...
	FCClaimSlot fc_claim[MAX_FP_ON_TARGET];
};
...
enum _sp {
	...
	SP_NON_CRIT_ATK_RATE,SP_CRITICAL_SKILL_DEF //2108
};

...
int32 npc_touch_areanpc(map_session_data* sd, int16 m, int16 x, int16 y, npc_data *nd)
{
	...
	case NPCTYPE_WARP:
		...
		if (sd->state.autoattack && sd->mapindex != nd->u.warp.mapindex && !sd->fp.tg.active)
			return 2;
...
static enum e_CASHSHOP_ACK npc_cashshop_process_payment(npc_data *nd, int32 price, int32 points, map_session_data *sd) {
	...
	switch(nd->subtype) {
		...
		case NPCTYPE_POINTSHOP:
			{
				char output[CHAT_SIZE_MAX];
				int32 points_to_use = 0; // Pointshop uses only cost[0] (#Playtime), not Kafra/Cash

				memset(output, '\0', sizeof(output));

				if (cost[1] < points_to_use || cost[0] < (price - points_to_use)) {
					sprintf(output, msg_txt(sd, 713), nd->u.shop.pointshop_str); // You do not have enough '%s'.
					clif_messagecolor(sd, color_table[COLOR_RED], output, false, SELF);
					return ERROR_TYPE_PURCHASE_FAIL;
				}

				if( !set_reg_num( nullptr, sd, add_str( nd->u.shop.pointshop_str ), nd->u.shop.pointshop_str, cost[0] - ( price - points_to_use ), nullptr ) ){
					return ERROR_TYPE_PURCHASE_FAIL;
				}

				sprintf(output, msg_txt(sd, 716), nd->u.shop.pointshop_str, cost[0] - (price - points_to_use)); // Your '%s' is now: %d
...
int32 npc_cashshop_buylist( map_session_data *sd, int32 points, std::vector<s_npc_buy_list>& item_list ){
	...
	for( i = 0; i < item_list.size(); i++ ) {
		...
		if( !pet_create_egg(sd,nameid) ) {
			...
			item_data *it = itemdb_search(nameid);
			if (it) {
				char msg_a[CHAT_SIZE_MAX];

				if (amount > 1) {
					snprintf(msg_a, sizeof(msg_a),
							"%s has purchased x%d %s from the %s!",
							sd->status.name, amount, it->ename.c_str(), nd->name);
				} else {
					snprintf(msg_a, sizeof(msg_a),
							"%s has purchased %s from the %s!",
							sd->status.name, it->ename.c_str(), nd->name);
				}

				clif_broadcast2(nullptr, msg_a, strlen(msg_a) + 1,
								0xFFFF63, FW_NORMAL, 12, 1, 0, ALL_CLIENT);
			}
		}
	}
...
uint8 npc_selllist(map_session_data* sd, int32 list_length, const PACKET_CZ_PC_SELL_ITEMLIST_sub* item_list)
{
	...
	for( i = 0; i < list_length; i++ )
	{
		...
		if (battle_config.rental_item_novalue && sd->inventory.u.items_inventory[idx].expire_time)
			...
			value = pc_calc_sell_price(sd, sd->inventory_data[idx], sd->inventory_data[idx]->value_sell);
...
const char* npc_parse_duplicate( char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, map_session_data* owner = nullptr ){
	...
	switch( type ) {
		...
		case NPCTYPE_MARKETSHOP:
			++npc_shop;
			safestrncpy( nd->u.shop.pointshop_str, dnd->u.shop.pointshop_str, sizeof( nd->u.shop.pointshop_str ) );
	...
	switch( mapflag ){
		...
		case MF_NOITEM:
			if( state ) {
				...
				std::shared_ptr<item_data> data = nullptr;
				
				char *str = aStrdup( w4 ), *temp = (char*)aMalloc(l +1);
				struct map_data *mapdata = map_getmapdata(m);
				
				//ShowError("3  size w4 %d mapdata->noitemlist.size %d str %s \n ", l, mapdata->noitemlist.size(),str);
				
				if(mapdata->noitemlist.size() > 0)
					mapdata->noitemlist.clear();

				while(i <= l){
					//ShowError("3 str[i] %c, i %d \n ", str[i], i);
					if (str[i] != ' ' && str[i] != '	' && str[i] != ',' && str[i] != '\0') {
						...
						if (!strcmp( temp, "IT_HEALING" ) || !strcmp(temp, "0"))
							...
							//ShowError("4 found item name %s \n",temp);
							data = item_db.search_aegisname( temp );
							if( data == nullptr ){
								ShowWarning("npc_parse_mapflag: Item name \"%s\" does not exist.\n    Mapflag noitem: At %s (file '%s', line '%d').\n", temp, mapname, filepath, strline(buffer,start-buffer));
								id = 0;
							} else {
								id = data->nameid;
							}
						} else {
							//ShowError("5 found item item %d \n",atoi(temp));
							id = atoi(temp);
							data = item_db.find(id);
							if( data == nullptr ){
						...
						if(id > 0) {
							k = 0;
							auto it = std::find_if(mapdata->noitemlist.begin(), mapdata->noitemlist.end(), [id] ( int const &v) {return v == id;});
							//ShowError("7 it %d id %d \n", it, id);
							if (it != mapdata->noitemlist.end()){
								//ShowError("8 found %d \n", it);
								if(id > 500){
									data = item_db.find(id);
									ShowWarning("npc_parse_mapflag: Item ID \"%d\" is being repeated.\n    Mapflag noitem: At %s (file '%s', line '%d') .\n", id, mapname, filepath, strline(buffer,start-buffer));
								} else {
									char item_type[NAME_LENGTH];
									switch(id) {
										case 0: safesnprintf(item_type, NAME_LENGTH, "IT_HEALING"); break;
										case 2: safesnprintf(item_type, NAME_LENGTH, "IT_USABLE"); break;
										case 4: safesnprintf(item_type, NAME_LENGTH, "IT_WEAPON"); break;
										case 5: safesnprintf(item_type, NAME_LENGTH, "IT_ARMOR"); break;
										case 6: safesnprintf(item_type, NAME_LENGTH, "IT_CARD"); break;
										case 10: safesnprintf(item_type, NAME_LENGTH, "IT_AMMO"); break;
										case 11: safesnprintf(item_type, NAME_LENGTH, "IT_DELAYCONSUME"); break;
										case 18: safesnprintf(item_type, NAME_LENGTH, "IT_CASH"); break;
										default: return strchr(start,'\n'); // hopefully doesn't run this
									}
									ShowWarning("npc_parse_mapflag: Item type \"%s(%d)\" is being repeated.\n    Mapflag noitem: At %s (file '%s', line '%d') .\n", item_type, id, mapname, filepath, strline(buffer,start-buffer));
								}
							} else {
								mapdata->noitemlist.push_back(id);
								//ShowError("10 not found, added %d mapdata->noitemlist.size %d \n", id, mapdata->noitemlist.size());
							}
						}
						j = 0;
						id = 0;

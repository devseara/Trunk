...
void mob_damage(mob_data *md, block_list *src, int32 damage)
{
	if (src != nullptr) { //Store total damage...
		...
		/* Overkill [DevSeara] */
        if (md->tdmg == damage) {
            if (damage > md->status.max_hp) {
				md->is_overkill = true;
				clif_specialeffect((block_list*)md, 1719, AREA);
            } else {
                md->is_overkill = false;
            }
        }
...
int32 mob_getdroprate(block_list *src, std::shared_ptr<s_mob_db> mob, int32 base_rate, int32 drop_modifier, mob_data* md, int32 factor)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			/* Overkill [DevSeara] */
			if( battle_config.overkill_mode){
				if (md && md->is_overkill) {
					drop_rate_bonus += battle_config.drops_overkill; // Apply a 30% bonus to the drop rate bonus
				}
			}

			/* Auto Attack Drop Rate Deduction [DevSeara] */
			if (sd && sd->sc.getSCE(SC_AUTOATTACK)) {
				int penalty = battle_config.aa_drops_penalty;

				if (pc_isvip(sd))
					penalty = max(0, penalty - 30);

				drop_rate_bonus -= penalty;
			}
...
int32 mob_dead(mob_data *md, block_list *src, int32 type)
{
	...
	aa_pending_flush_map_for_target(md);
	...
	{ // Item Drop
		...
		if(sd) {
			...
			for (const auto &it : sd->add_drop) {
					...
					(it.class_ == CLASS_ALL || it.class_ == status->class_)) //Matched class
				{
					...
					if (sd && sd->state.autoattack && drop_rate > 1)
		...
		for( const std::shared_ptr<s_mob_drop>& entry : md->db->dropitem ){
			...
			/* Fake Drop Rate [DevSeara] */
			int threshold = battle_config.fake_drop_rate * 100;
			int rdrop_rate = drop_rate;
			...
			if (first_sd != nullptr && !first_sd->fp.is_fake_player && entry->rate <= battle_config.rare_drop_announce) {
 				char message[128];
				sprintf(message, msg_txt(nullptr, 541), first_sd->status.name, md->name, it->ename.c_str(), (float)rdrop_rate / 100);
	...
	if( mvp_sd != nullptr ){
		...
		if( !(map_getmapflag(m, MF_NOMVPLOOT) || type&1) ) {
			...
			for( const std::shared_ptr<s_mob_drop>& entry : mdrop ){
				...
				if(temp<=battle_config.rare_drop_announce && !mvp_sd->fp.is_fake_player) {
					char message[128];
...
uint64 MobDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "DisplayName")) {
		...
		if (!this->asString(node, "DisplayName", name))
			return 0;

		if (name.size() > DISPLAY_NAME_MAX) {
			this->invalidWarning(node["DisplayName"], "DisplayName \"%s\" exceeds maximum of %d UTF-8 bytes, capping...\n", name.c_str(), DISPLAY_NAME_MAX);
			name.resize(DISPLAY_NAME_MAX);
		}

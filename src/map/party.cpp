...
int32 party_recv_message( int32 party_id, uint32 account_id, const char *mes, size_t len ){
	...
	map_session_data* from_sd = map_id2sd(account_id);
		if (!from_sd) return 0;

	// --- Réaction des fake players du groupe ---
	for (int i = 0; i < MAX_PARTY; ++i) {
		struct party_member& m = p->party.member[i];
		if (!m.account_id) continue;

		map_session_data* bot_sd = map_id2sd(m.char_id);
		if (!bot_sd || !bot_sd->fp.is_fake_player) continue;

		// trim rapide
		std::string_view sv{ mes, len };
		constexpr char ws[] = " \t\n\r\f\v";
		std::string_view so;
		...
		if (fakechat_try_handle_message_step2(bot_sd, from_sd, std::string(so), /*is_private=*/false,
			reply, delay_ms, intent,
			FCChannel::PARTY /*hint*/))
		{
			ShowInfo("[FakeChat][Party] from='%s' -> bot='%s' intent=%s delay=%dms reply=\"%s\"",
				from_sd->status.name, bot_sd->status.name, intent.c_str(), delay_ms, reply.c_str());
		}
	...
	if (p && p->party.item&2 && (first_charid || !(battle_config.party_share_type&1))) {
		...
		if (battle_config.party_share_type&2) { // Round Robin
			...
			while (count > 0) { //Pick a random member.
				...
				if (psd[count - 1]->fp.is_fake_player) {
					count--;
					continue;
				}

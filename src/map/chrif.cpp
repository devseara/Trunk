	...
	if( flag&CSAVE_QUITTING ){
		if( sd->goldpc_tid != INVALID_TIMER ){
			...
			if( td != nullptr ){
				...
				// Seconds already elapsed in this interval (round down so we don't give free time)
				int32 elapsed_sec = (int32)( remaining <= 0 ? battle_config.feature_goldpc_time : ( battle_config.feature_goldpc_time - ( remaining / 1000 ) ) );
				elapsed_sec = cap_value( elapsed_sec, 0, battle_config.feature_goldpc_time );
				pc_setreg2( sd, GOLDPC_SECONDS_VAR, elapsed_sec );
...
void chrif_on_ready(void) {
	...
	if (!char_init_done)
	{
		...
		size_t n = connect_fake_players_from_init();
		char_init_done = true;
	}
}

...
// Banquet/WARP custom item-color list. Variable payload entries are encoded as
// uint16 name length (including NUL), UTF-8 name bytes, then uint32 RRGGBB.
struct PACKET_ZC_ITEM_COLOR_LIST {
	uint16 packetType;
	uint16 packetLength;
	uint32 count;
} __attribute__((packed));
DEFINE_PACKET_HEADER(ZC_ITEM_COLOR_LIST, 0x0bdd);

// Banquet/WARP custom extended-name side packet. Stock name and spawn packets
// stay unchanged; the patched client keys these long values by shortName.
struct PACKET_ZC_EXT_NAME {
	uint16 packetType;
	uint16 packetLength;
	uint32 magic;
	uint32 gid;
	char shortName[NAME_LENGTH];
	char name[DISPLAY_NAME_LENGTH];
	char party_name[DISPLAY_NAME_LENGTH];
	char guild_name[DISPLAY_NAME_LENGTH];
	char position_name[DISPLAY_NAME_LENGTH];
	char title_name[DISPLAY_NAME_LENGTH];
	char party_name_short[NAME_LENGTH];
	char guild_name_short[NAME_LENGTH];
	char position_name_short[NAME_LENGTH];
	char title_name_short[NAME_LENGTH];
	uint8 type;       // 0=PC, 1=MOB, 2=NPC, 3=portal NPC
	uint32 name_color; // Reserved for the separate player-name-color patch.
} __attribute__((packed));
static_assert(sizeof(PACKET_ZC_EXT_NAME) == 777, "0x0BB7 packet layout must match the WARP EXTN parser");
static_assert(offsetof(PACKET_ZC_EXT_NAME, type) == 772, "0x0BB7 type offset mismatch");
static_assert(offsetof(PACKET_ZC_EXT_NAME, name_color) == 773, "0x0BB7 color offset mismatch");
DEFINE_PACKET_HEADER(ZC_EXT_NAME, 0x0bb7);

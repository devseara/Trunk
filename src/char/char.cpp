...
int32 char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	uint8 is_fake = 0;
	{
		SqlStmt stmt_fake{ *sql_handle };

		if (SQL_ERROR == stmt_fake.Prepare(
			"SELECT `is_fake` FROM `%s` WHERE `account_id` = '%d' AND `char_id` = '%d'",
			schema_config.char_db, p->account_id, p->char_id)
			|| SQL_ERROR == stmt_fake.Execute()
			|| SQL_ERROR == stmt_fake.BindColumn(0, SQLDT_INT8, &is_fake, 0, nullptr, nullptr)
			) {
			SqlStmt_ShowDebug(stmt_fake);
		}
	}
	if (is_fake) return 0;
	...
	if (
		...
		strcmp(p->display_name, cp->display_name) != 0
	)
	{
		char escaped_display_name[DISPLAY_NAME_LENGTH * 2 + 1];
		Sql_EscapeStringLen(sql_handle, escaped_display_name, p->display_name, strnlen(p->display_name, DISPLAY_NAME_LENGTH));
		...
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `class`='%d',"
			...
			p->body_direction, p->disable_call, p->disable_partyinvite, p->disable_showcostumes, escaped_display_name,
			p->account_id, p->char_id) )
		{
			Sql_ShowDebug(sql_handle);
			errors++;
...
int32 char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == stmt.Prepare( "SELECT "
		...
		"`max_ap`,`ap`,`trait_point`,`pow`,`sta`,`wis`,`spl`,`con`,`crt`,"
		"`inventory_slots`,`body_direction`,`disable_call`,`last_instanceid`,`disable_partyinvite`,`disable_showcostumes`,`display_name`"
	...
	||	SQL_ERROR == stmt.BindColumn(78, SQLDT_STRING, &p->display_name, sizeof(p->display_name))
	)
	{
		SqlStmt_ShowDebug(stmt);
		return 0;
...
enum e_char_del_response char_delete(struct char_session_data* sd, uint32 char_id){
	...
	if( charserv_config.char_config.char_del_delay > 0 && ( !delete_date || delete_date > time(nullptr) ) ){ // not queued or delay not yet passed
		...
		// @security: block char deletion when #ACCOUNTLOCK=1
	if( charserv_config.char_config.security_lock_block_delete ){
		char esc_key[32];
		Sql_EscapeStringLen(sql_handle, esc_key, "#ACCOUNTLOCK", sizeof(esc_key) - 1);
		if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `value` FROM `%s` WHERE `account_id` = '%u' AND `key` = '%s' LIMIT 1",
			schema_config.acc_reg_num_table, sd->account_id, esc_key) ){
			Sql_ShowDebug(sql_handle);
			...
			Sql_FreeResult(sql_handle);
			if( data && atoi(data) != 0 ){
				ShowInfo("Char deletion aborted: %s, Account ID: %u - Security lock active (#ACCOUNTLOCK)\n", name, sd->account_id);
				return CHAR_DELETE_SECURITY_LOCK;
			}
		} else {
			Sql_FreeResult(sql_handle);
		}
...
void char_set_defaults(){
	...
	charserv_config.char_config.security_lock_block_delete = true; // @security: block char deletion when #ACCOUNTLOCK=1
...
bool char_config_read(const char* cfgName, bool normal){
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		} else if (strcmpi(w1, "security_lock_block_delete") == 0) {
			charserv_config.char_config.security_lock_block_delete = (bool)config_switch(w2);
